...
module.exports.register = function () {
  this.on("contentAggregated", ({ contentAggregate }) => {
    const docsBucket = contentAggregate.find((b) => b.name === "evolveum-docs");
    ...
    const rewrite = (content) =>
      content
        .replace(/xref:evolveum-midpoint::([^[\n]*)\[/g, "xref:ROOT:$1[")
        .replace(/xref:([^:[\n]+)\[/g, "xref:evolveum-docs::$1[");
    ...
    for (const bucket of contentAggregate) {
      if (bucket.name !== "evolveum-midpoint") continue;
